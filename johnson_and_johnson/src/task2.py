...
duckdb.sql(
    (
        """
        SELECT round(sum(cost_eur), 2) as total_cost
        FROM 'input_data/maintenance_events/maintenance_events.csv'
        """
    )
).show()
...
duckdb.sql(
    (
        """
        SELECT round(sum(downtime_min), 2) as total_downtime
        FROM 'input_data/maintenance_events/maintenance_events.csv'
        """
    )
).show()
...
duckdb.sql(
    (
        """
        SELECT count(*) as unplanned_breakdowns
        FROM 'input_data/maintenance_events/maintenance_events.csv'
...
duckdb.sql(
    (
        """
        SELECT reason, round(avg(downtime_min), 2) as avg_downtime_per_event
        FROM 'input_data/maintenance_events/maintenance_events.csv'
        GROUP BY reason
        """
    )
